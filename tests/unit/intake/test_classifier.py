...
from launcher.intake.repo_classifier import (
...
class TestClassifyRepo:
    ...
    def test_non_python_language_eligible_under_default_config(self):
        """TC-4071: Default config no longer requires Python — JavaScript repos are eligible."""
        repo = _make_repo(language="JavaScript")
        result = classify_repo(repo)
        # require_python default is now False, require_language is None → language check skipped
        assert result.decision == "eligible"
        assert not any("JavaScript" in r for r in result.reasons)

    def test_no_language_eligible_under_default_config(self):
        """TC-4071: Default config no longer requires any language — no-language repos are eligible
        as long as other checks (license, readme, size) pass."""
        repo = _make_repo(language="")
        result = classify_repo(repo)
        # language check skipped under default config (require_python=False, require_language=None)
        # other checks: MIT license ✓, description+size ✓ → eligible
        assert result.decision == "eligible"
        assert not any("no primary language" in r for r in result.reasons)

    def test_deprecated_require_python_still_works_when_explicit(self):
        """TC-4071: Explicit require_python=True still flags non-Python as needs_review."""
        config = ClassifierConfig(require_python=True)
        repo = _make_repo(language="JavaScript")
        result = classify_repo(repo, config=config)
        assert result.decision == "needs_review"
...
class TestClassifyInspection:
    def test_python_repo_inspection_eligible(self):
        inspection = {
            "repo": _make_repo(),
            "platform": "python",
            "acquisition": {
    ...
    def test_non_python_repo_inspection_eligible(self):
        inspection = {
            "repo": _make_repo(language="Java", full_name="org/JavaRepo"),
            "platform": "java",
            "acquisition": {
                "repo_signals": {
                    "readme_present": True,
                    "is_empty_clone": False,
                    "detected_manifest_files": ["pom.xml"],
                }
            },
            "shared_facts": {
                "package_name": "org.example:java-repo",
                "license_type": "MIT",
    ...
    def test_inspection_without_readme_is_ineligible(self):
        inspection = {
            "repo": _make_repo(),
            "platform": "python",
            "acquisition": {
        ...
        result = classify_inspection(inspection)
        assert result.decision == "ineligible"
        assert any("README" in reason for reason in result.reasons)
