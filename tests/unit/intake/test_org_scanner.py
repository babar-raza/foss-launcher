...
from launcher.intake.org_scanner import (
    GITHUB_API_BASE,
    RateLimitError,
    ScannerError,
    _parse_link_header,
...
class TestScanOrgs:
    ...
    def test_multiple_orgs(self, mock_get, tmp_path):
        ...
        assert len(result) == 2
        assert {r["full_name"] for r in result} == {"org1/A", "org2/B"}

    @patch("launcher.intake.org_scanner.requests.get")
    def test_force_rescan_processes_seen_repos(self, mock_get):
        """TC-4073: force_rescan=True causes previously-seen repos to be processed again."""
        repos = [_make_repo(name="R1", full_name="org1/R1")]
        mock_get.return_value = _mock_response(json_data=repos)
        seen = {"org1/R1"}  # repo already in seen set
        result = scan_org("org1", seen_repos=seen, force_rescan=True, rate_limiter=lambda _: None)
        # With force_rescan=True, the seen repo is NOT skipped
        assert len(result) == 1
        assert result[0]["full_name"] == "org1/R1"

    @patch("launcher.intake.org_scanner.requests.get")
    def test_default_skips_seen_repos(self, mock_get):
        """TC-4073: force_rescan=False (default) keeps the existing skip behavior."""
        repos = [_make_repo(name="R1", full_name="org1/R1")]
        mock_get.return_value = _mock_response(json_data=repos)
        seen = {"org1/R1"}
        result = scan_org("org1", seen_repos=seen, rate_limiter=lambda _: None)
        # force_rescan defaults to False — seen repo is skipped
        assert len(result) == 0

    @patch("launcher.intake.org_scanner.requests.get")
    def test_force_rescan_scan_orgs(self, mock_get, tmp_path):
        """TC-4073: force_rescan=True passed to scan_orgs is propagated to scan_org."""
        # Pre-populate state
        repos = [_make_repo(name="R1", full_name="org1/R1")]
        mock_get.return_value = _mock_response(json_data=repos)
        scan_orgs(["org1"], state_dir=tmp_path / "intake", rate_limiter=lambda _: None)

        # Force rescan — same repos should be returned again
        mock_get.return_value = _mock_response(json_data=repos)
        result = scan_orgs(
            ["org1"],
            state_dir=tmp_path / "intake",
        ...
        assert len(result) == 1, "force_rescan=True should return repos even if they were seen before"
