---
id: TC-1103
title: W5 LLM Prompt Update for Limitations + W7 Check Refinement
status: Done
agent: Agent-D
owner: Agent-D
priority: P1
...
# TC-1103 — W5 LLM Prompt Update for Limitations + W7 Check Refinement

## Objective
Enhance W5 SectionWriter's LLM prompt to explicitly instruct creation of Limitations sections when required, and refine W7 ContentReviewer's limitation honesty check to be page-type specific, reducing false positives from 18 to ~3-5.
...
- `specs/35_w7_content_reviewer.md` — ContentReviewer checks and severity levels
...
- **D-102**: Refine W7 technical_accuracy.py check_7_limitation_honesty to be page-type specific
...
- Other W7 checks beyond check_7_limitation_honesty
- W8 LinkerPatcher or downstream workers
...
- `src/launch/workers/w7_content_reviewer/checks/technical_accuracy.py` — W7 technical accuracy checks
- `product_facts.json` — claim_groups.limitations list
- `page_plan.json` — required_headings per page
- Existing test files for W5 and W7
...
- Modified W7 technical_accuracy check with page-type specific severity
...
### Step 3: Analyze existing W7 limitation check
- Read `src/launch/workers/w7_content_reviewer/checks/technical_accuracy.py`
- Locate `_check_7_limitation_honesty()` function
- Understand current severity logic and page_role usage

### Step 4: Implement D-102 (W7 Check Refinement)
...
- Create/update `tests/unit/workers/w7_content_reviewer/test_checks.py`:
...
### FM-3: W7 check still produces false positives
...
**Spec/Gate**: Spec 35 (W7 ContentReviewer), Gate 15 (ContentReviewer)
...
### FM-5: Regression in existing W5/W7 behavior
...
5. W7 check returns error severity for overview/comprehensive_guide/api_overview pages
6. W7 check returns empty list for index/toc/getting_started/installation/faq/troubleshooting/how_to pages
7. W7 check returns warn severity for other page types not in explicit lists
...
- Modified `src/launch/workers/w7_content_reviewer/checks/technical_accuracy.py` with page-type specific check
- Test file `tests/unit/workers/test_w5_llm_generation.py` with prompt logic tests
- Test file `tests/unit/workers/w7_content_reviewer/test_checks.py` with check refinement tests
- Evidence package:
  - `reports/agents/agent_d/TC-CREV-D-TRACK2/plan.md`
  - `reports/agents/agent_d/TC-CREV-D-TRACK2/changes.md`
  - `reports/agents/agent_d/TC-CREV-D-TRACK2/evidence.md`
...
- [ ] W7 check page-type specific (error/skip/warn based on page_role)
- [ ] Test coverage ≥4 new tests (2 for W5 prompt, 2+ for W7 check)
...
- W7 ContentReviewer operational (TC-1100)
- Product facts with claim_groups structure (TC-410)
- Test infrastructure for W5 and W7 workers
...
2. Unit tests for W7 check refinement:
...
3. Integration test:
   ...
   - Run W7 on sample pages → verify check behavior matches expected severity
4. Regression test:
...
```bash
PYTHONHASHSEED=0 .venv/Scripts/python.exe -m pytest tests/unit/workers/test_tc_440_section_writer.py tests/unit/workers/w7_content_reviewer/test_checks.py -xvs
```
...
- Run W7 ContentReviewer on drafts with missing Limitations
...
- Output: W5 draft_sections with enhanced LLM-generated content, W7 review results with page-type specific checks
...
- W7 ContentReviewer consumes W5 drafts and applies page-type specific checks
- No changes to artifact schemas (page_plan.json, product_facts.json)
- No changes to W4 → W5 → W7 → W6 pipeline flow
- Backward compatible: optional parameters with None defaults
- Test suite validates integration: 93 tests pass (W5 + W7)
