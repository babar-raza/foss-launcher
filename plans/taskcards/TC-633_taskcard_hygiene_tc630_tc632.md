...
These paths cover only the three taskcards with validation failures, the INDEX file that needs their entries, and the STATUS_BOARD which is auto-generated by Gate C.
...
1. **Failure**: STATUS_BOARD.md regeneration fails or overwrites manual edits
   - **Detection**: Gate C fails or STATUS_BOARD.md has unexpected diff
   - **Fix**: STATUS_BOARD.md is auto-generated by Gate C; do not manually edit
   - **Spec/Gate**: Gate C (Status board generation)

2. **Failure**: Spec reference still broken after fix
   - **Detection**: Gate A2 still reports broken spec reference error
   - **Fix**: Double-check the corrected spec path exists using `ls specs/<path>` before updating taskcard
   - **Spec/Gate**: Gate A2 (Plans validation - zero warnings)

3. **Failure**: Added sections do not satisfy validator requirements
   - **Detection**: Gate A2 or B still reports missing section errors
   - **Fix**: Review plans/taskcards/00_TASKCARD_CONTRACT.md for exact section heading requirements (case-sensitive, markdown level)
   - **Spec/Gate**: Gate A2, Gate B (Taskcard validation)
