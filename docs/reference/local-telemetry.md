...
**Launcher mapping:** See [specs/16_local_telemetry_api.md](../../specs/16_local_telemetry_api.md) for how the launcher must use this API.
...
### Launcher-specific notes (this repo)
- The launcher requires **deterministic** `run_id` values (see `specs/16_local_telemetry_api.md`).
- Telemetry transport failures are **non-fatal** for the launcher, but must be buffered to `RUN_DIR/telemetry_outbox.jsonl`.
- The launcher MUST associate the final PR commit SHA to the parent run and all pre-commit child runs.
...
- **Launcher usage spec:** [specs/16_local_telemetry_api.md](../../specs/16_local_telemetry_api.md)
- **System Architecture:** [specs entry point](../../specs/README.md)
- **Deployment Guide:** [architecture overview](../../docs/architecture.md)
