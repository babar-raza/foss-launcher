...
def normalize_validation_report(
    ...
    """
    Normalize validation report for determinism.

    Replaces:
    - Absolute run_dir paths → "<RUN_DIR>"
    - Absolute repo root paths → "<REPO_ROOT>"
    - Removes timestamps (if any in report body)
    - Stable sorting of lists

    Args:
        report: Raw validation report dict
        run_dir: Run directory path for replacement
    ...
    # Collect all possible path representations to replace
    # This handles both resolved and unresolved paths, Windows and Unix separators
    run_dir_variants = [
        str(run_dir.resolve()),
        str(run_dir),
    ]
    ...
    # Add forward-slash versions for Windows paths
    for variant in list(run_dir_variants):
        if "\\" in variant:
            run_dir_variants.append(variant.replace("\\", "/"))
    for variant in list(repo_root_variants):
        if "\\" in variant:
            repo_root_variants.append(variant.replace("\\", "/"))

    # Remove duplicates and sort by length (longest first for proper replacement)
    run_dir_variants = sorted(set(run_dir_variants), key=len, reverse=True)
    repo_root_variants = sorted(set(repo_root_variants), key=len, reverse=True)

    # Deep copy to avoid mutation
    normalized = copy.deepcopy(report)

    # Recursively replace paths in all strings
    def replace_paths(obj):
        if isinstance(obj, str):
            # Replace run_dir first (more specific, longest first)
            for variant in run_dir_variants:
                obj = obj.replace(variant, "<RUN_DIR>")
            # Replace repo root
            for variant in repo_root_variants:
                obj = obj.replace(variant, "<REPO_ROOT>")
    ...
    normalized = replace_paths(normalized)

    # Remove timestamps if present in report body (NOT events.ndjson)
    if "generated_at" in normalized:
        del normalized["generated_at"]
    if "timestamp" in normalized:
        del normalized["timestamp"]

    # Stable sort of gate_results by name
    if "gates" in normalized:
        normalized["gates"] = sorted(
            normalized["gates"],
            key=lambda g: g.get("name", "")
        )

    # Stable sort of issues by file + line + message
    if "issues" in normalized:
        normalized["issues"] = sorted(
            normalized["issues"],
            key=lambda i: (
                i.get("location", {}).get("path", "") if isinstance(i.get("location"), dict) else "",
                i.get("location", {}).get("line", 0) if isinstance(i.get("location"), dict) else 0,
                i.get("message", "")
            )
        )
...
def compute_canonical_hash(json_path: Path) -> str:
    ...
    Args:
        json_path: Path to JSON artifact
...
def execute_validator(run_dir: Path, run_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Normalize for determinism (TC-702)
    validation_report = normalize_validation_report(validation_report, run_dir)
