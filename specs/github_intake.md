# GitHub Organization Intake System
...
|-----------|--------|---------------|
| **Org Scanner** | `src/launcher/intake/org_scanner.py` | Paginated GitHub API discovery with rate limiting and state persistence |
| **Repo Classifier** | `src/launcher/intake/repo_classifier.py` | Deterministic eligibility classification (eligible / ineligible / needs_review) |
| **Config Generator** | `src/launcher/intake/config_generator.py` | YAML pilot config generation from repo metadata with dedup |
| **Scheduler** | `src/launcher/intake/scheduler.py` | Priority queue, batch mode, dry-run report |
| **CLI** | `src/launcher/cli/main.py` (intake sub-app) | `launch intake scan`, `launch intake classify`, `launch intake generate` |
...
Reads a base template (defaults to built-in `_DEFAULT_TEMPLATE`)
and fills in:
...
- `repo_url`: from repo `html_url`
- `github_ref`: from repo `default_branch` (typically `main`)

### 6.2 Dedup

Uses a two-tier dedup strategy:
1. **Slug-based**: checks if `{output_dir}/{slug}.yaml` already exists (O(1))
2. **Index-based**: maintains `.dedup_index.json` mapping `repo_url` → `slug` for fast URL-based dedup (O(1) after initial load)
3. **Fallback**: if index is missing or corrupt, falls back to full YAML scan
