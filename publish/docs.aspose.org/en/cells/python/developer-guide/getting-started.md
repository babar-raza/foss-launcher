...
Install Aspose.Cells FOSS using pip to begin working with spreadsheet files in Python. The library provides a minimal, focused API for reading, writing, and manipulating Excel-compatible formats without external dependencies like openpyxl or pandas.

Create a new workbook and access the first worksheet to begin editing. The `Workbook` class serves as the entry point, and `worksheets[0]` retrieves the default worksheet.
...
# Assign a formula to another cell
worksheet.cells["B1"].formula = "=LEN(A1)"
