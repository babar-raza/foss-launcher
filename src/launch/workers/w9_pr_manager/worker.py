...
def generate_pr_body(
    ...
    if issues:
        ...
        for issue in sorted(issues, key=lambda i: (i.get("severity", ""), str(i.get("code", "")))):
...
def execute_pr_manager(
    ...
    try:
        # TC-1033: Load required artifacts via ArtifactStore
        _store = ArtifactStore(run_dir=run_layout.run_dir)
        try:
            patch_bundle = _store.load_artifact("patch_bundle.json", validate_schema=False)
        except FileNotFoundError:
            raise PRManagerMissingArtifactError(
                f"patch_bundle.json not found at {run_layout.artifacts_dir / 'patch_bundle.json'}"
            )

        try:
            validation_report = _store.load_artifact("validation_report.json", validate_schema=False)
        except FileNotFoundError:
            raise PRManagerMissingArtifactError(
                f"validation_report.json not found at {run_layout.artifacts_dir / 'validation_report.json'}"
            )
...
"""
        ...
        # Check for offline mode (also auto-enable for local validation profile)
        ...
        if approval_marker_path.exists():
            ...
            error_msg = (
                f"AG-001 approval gate violation: Branch creation requires explicit user approval. "
                f"Approval marker file not found: {approval_marker_path}"
            )
            logger.error(
                "pr_manager_ag001_approval_required",
                run_id=run_id,
                message=error_msg,
