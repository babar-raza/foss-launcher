"""Deterministic provenance tracking for autopilot artifact reuse.

TC-3000: Foundation module for the autopilot phase selection system.
...
TC-3250: ENGINE_VERSION + compute_interpretation_signature for two-layer
store keying (raw W1 layer vs derived W2/W3/W4 layer).
...
# ---------------------------------------------------------------------------
# Interpretation signature (TC-3250)
# ---------------------------------------------------------------------------
...
def compute_interpretation_signature(run_config: Dict[str, Any]) -> str:
    """Compute a 12-character hex interpretation signature for W2/W3/W4 keying.

    The signature changes when any of the following change:
    - ``ENGINE_VERSION`` (bump when W2/W3/W4 logic changes)
    - ``ruleset_version`` (affects W2/W4 content rules)
    - ``templates_version`` (affects W4 IA structure)
