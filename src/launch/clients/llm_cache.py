...
Write-fence owner: TC-500 / sub-task TC-2409.

Purpose
-------
Speed up reruns, ``launch resume`` restarts, and iterative gate-fix cycles by
returning prior LLM responses from disk instead of making network calls for
identical requests.  The cache is **opt-in** and has zero effect when disabled
(the default).
...
Spec: docs/reference/config.md — LLM Disk Cache section
"""
