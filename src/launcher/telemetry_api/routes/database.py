...
_UPDATABLE_FIELDS = frozenset({
    ...
    "commit_hash", "commit_source", "commit_author", "commit_timestamp",
})
...
class TelemetryDatabase:
    ...
    def create_run(self, run_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new run record (idempotent by event_id)."""
        with self._get_connection() as conn:
            cursor = conn.cursor()
        ...
        with self._get_connection() as conn:
            ...
            for field, value in update_data.items():
                if field not in _UPDATABLE_FIELDS:
                    logger.warning("update_run: skipping unknown field %r", field)
                    continue
    ...
    def batch_create_runs(
        ...
        Returns (results, created_count, existing_count).
        If transactional=True, rolls back all on any failure.
        """
        with self._get_connection() as conn:
            cursor = conn.cursor()
            results = []
            ...
            try:
                ...
                for run_data in run_data_list:
                    ...
                    if existing:
                        existing_count += 1
                        results.append(self._row_to_dict(existing))
                        continue
                ...
                conn.commit()
                return (results, created_count, existing_count)
