...
1. Load extracted_claims.json from
2. Load discovered_docs.json and discovered_examples.json from
...
W2.2 Map claims to evidence in docs and examples
"""
...
from launcher.util.logging import get_logger
...
# Evidence scoring weights (tuned heuristic from optimization).
...
# Source-type weights for citation quality scoring (ported from v1 repo_profiler).
# Higher weight = more authoritative evidence source.
...
def infer_source_type(path: str) -> str:
    """Infer the source type of a file from its path."""
    p = path.lower()
    if "readme" in p:
        return "readme"
...
def score_citation_quality(
    ...
    """Compute [0, 1] quality score for a claim's citations.

    Uses SOURCE_TYPE_WEIGHTS to score each citation by its source type.
    Returns the maximum weight across all citations (best evidence wins).
    """
    if not citations:
        return 0.0
    scores: list[float] = []
    for cit in citations:
        src_type = cit.get("source_type") or infer_source_type(cit.get("path", ""))
        scores.append(SOURCE_TYPE_WEIGHTS.get(src_type, 0.3))
...
def compute_text_similarity(text1: str, text2: str, _tokens2_cache=None) -> float:
    ...
    """
    from launcher.shared.embeddings import tokenize
...
def _load_and_tokenize_files(
    ...
    """
    from launcher.shared.embeddings import precompute_token_cache
...
def map_evidence(
    ...
    - Reads extracted_claims.json from
    - Reads discovered_docs.json and discovered_examples.json from
    ...
    # Load extracted_claims.json from
    ...
    # Load discovered_docs.json from
    ...
    # Load discovered_examples.json from
    ...
    evidence_map = {
        ...
        'contradictions': [],  # Contradiction detection not implemented in
        'metadata': {
