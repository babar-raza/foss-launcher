...
def _load_allowlist(allowlist_path: Optional[Path] = None) -> list[str]:
    """Load network allowlist from configs/network_allowlist.yaml."""
    if allowlist_path is None:
        # src/launcher/clients/http.py -> go up 4 levels to reach repo root
        repo_root = Path(__file__).parent.parent.parent.parent
        allowlist_path = repo_root / "configs" / "network_allowlist.yaml"
...
def _validate_url(url: str, allowlist_path: Optional[Path] = None) -> None:
    ...
    if not _is_host_allowed(host, allowlist):
        raise NetworkBlockedError(
            ...
            f"Add to configs/network_allowlist.yaml or use a different endpoint.",
            host=host,
