...
class WorkerCheckpoint:
    """Per-worker checkpoint with SHA-256 content hash for edit detection."""

    checkpoint_id: str
    worker: str
    run_id: str
    created_at: str  # ISO8601 UTC
    artifact_path: str  # relative path from run_dir
    content_hash: str  # SHA-256 of artifact file bytes
...
def write_worker_checkpoint(
    ...
    """Write a per-worker checkpoint JSON and return the WorkerCheckpoint record."""
    import datetime as _datetime
    checkpoint_id = f"{worker}_{_datetime.datetime.now(_datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%f')}"
    content_hash = hashlib.sha256(artifact_path.read_bytes()).hexdigest()
    rel_path = str(artifact_path.relative_to(run_dir)) if artifact_path.is_relative_to(run_dir) else str(artifact_path)
    ...
    cp_file.write_text(
        json.dumps(dataclasses.asdict(cp), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return cp
...
def load_worker_checkpoint(
    ...
    """Load a WorkerCheckpoint by ID. Returns None if not found."""
    cp_file = run_dir / "worker_checkpoints" / f"{checkpoint_id}.json"
...
def restore_worker_checkpoint(
    ...
    """Verify SHA-256 hash matches artifact. Returns True if intact, False if edited."""
    artifact = run_dir / checkpoint.artifact_path
    if not artifact.exists():
        return False
    current_hash = hashlib.sha256(artifact.read_bytes()).hexdigest()
    return current_hash == checkpoint.content_hash
...
def backup_output_files(
    ...
    *,
    ...
    """Copy listed files in run_dir to <filename><backup_suffix>.

    Only backs up files that exist. Returns list of backup paths created.
    Safe to call even if run_dir is missing (returns empty list).
    """
    ...
    for name in filenames:
        src = run_dir / name
        if src.exists():
            dst = run_dir / (name + backup_suffix)
...
def restore_from_backup(
    ...
    *,
    ...
    """Restore files from their <filename><backup_suffix> copies.

    Overwrites the target if it exists. Removes the backup after restoring.
    Returns list of paths restored. Skips silently if backup does not exist.
    """
    if not run_dir.is_dir():
        return []
    restored: list[Path] = []
    for name in filenames:
        bak = run_dir / (name + backup_suffix)
        if not bak.exists():
            continue
        dst = run_dir / name
        ...
        bak.unlink()
        restored.append(dst)
        logger.debug("Restored %s from backup", dst.name)
    return restored
